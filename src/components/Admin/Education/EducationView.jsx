...
const EducationView = () => {
  ...
  const { currentEducation, isLoading, error, getById, deleteEducation, clearError, clearCurrent } = useEducation();
  ...
  const fetchData = async () => {
    try {
      ...
      setLocalError('Failed to load education entry');
    }
  };
  ...
  if (error || localError || !currentEducation) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <AlertCircle size={48} className="text-red-400 mx-auto mb-4" />
          <p className="text-red-400">{error || localError || 'Education entry not found'}</p>
          <button
            onClick={() => {
              clearError();
              navigate('/admin/education');
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <div className="flex gap-2">
          <Link
            to={`/admin/education/edit/${currentEducation._id}`}
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex items-start gap-6">
            ...
            >
              <GraduationCap size={40} className="text-[#27CBCB]" />
            </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      {currentEducation.institution || 'N/A'}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <MapPin size={16} className="text-[#27CBCB]" />
                    <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      {currentEducation.location || 'N/A'}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <Calendar size={16} className="text-[#27CBCB]" />
                    <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
        ...
        {/* Description */}
        {currentEducation.description && currentEducation.description.length > 0 && (
          <div className={`rounded-xl p-6 border
            ${isDark ? 'bg-[#111111] border-gray-800' : 'bg-white border-gray-200'}`}
          >
            ...
            <ul className="space-y-2">
              {currentEducation.description.map((item, index) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  {formatDate(currentEducation.createdAt)}
                </p>
              </div>
            </div>
            <div className={`flex items-center gap-3 p-3 rounded-lg
            ...
            >
              ...
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  {formatDate(currentEducation.updatedAt)}
                </p>
              </div>
            </div>
          </div>
