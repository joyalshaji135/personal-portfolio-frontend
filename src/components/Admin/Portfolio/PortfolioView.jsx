...
const PortfolioView = () => {
  ...
  const { currentPortfolio, isLoading, error, getById, deletePortfolio, clearError, clearCurrent } = usePortfolio();
  ...
  if (error || localError || !currentPortfolio) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <AlertCircle size={48} className="text-red-400 mx-auto mb-4" />
          <p className="text-red-400">{error || localError || 'Portfolio not found'}</p>
          <button
            onClick={() => {
              clearError();
              navigate('/admin/portfolio');
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <div className="flex gap-2">
          <Link
            to={`/admin/portfolio/edit/${currentPortfolio._id}`}
      ...
      <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    {currentPortfolio.titleName || 'N/A'}
                  </h2>
                  <div className="flex items-center gap-2 mt-1">
                    <User size={16} className="text-[#27CBCB]" />
                    <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      Role: {currentPortfolio.role?.name || 'N/A'}
                    </p>
                  </div>
                </div>
              </div>
        ...
        {/* Taglines */}
        {currentPortfolio.taglines && currentPortfolio.taglines.length > 0 && (
          <div className={`rounded-xl p-6 border
            ${isDark ? 'bg-[#111111] border-gray-800' : 'bg-white border-gray-200'}`}
          >
            ...
            <div className="flex flex-wrap gap-2">
              {currentPortfolio.taglines.map((tagline, index) => (
