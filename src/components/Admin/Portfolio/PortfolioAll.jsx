...
const PortfolioAll = () => {
  const { portfolios, isLoading, error, getAll, deletePortfolio, clearError } = usePortfolio();
  ...
  const filteredData = portfolios.filter(item => {
    const matchesSearch = 
      ...
      item.taglines?.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected portfolios?`)) {
      setDeleteLoading(true);
      try {
        ...
        alert('Failed to delete selected portfolios. Please try again.');
      } finally {
        setDeleteLoading(false);
      }
    }
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Briefcase size={18} className="text-[#27CBCB]" />
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Roles</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {new Set(portfolios.map(p => p.role?._id)).size}
            </span>
          </div>
        </div>
        <div className={`rounded-xl p-4 border
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Taglines</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {portfolios.reduce((acc, p) => acc + (p.taglines?.length || 0), 0)}
            </span>
          </div>
        </div>
      </div>
        ...
        {selectedItems.length > 0 && (
          ...
          >
            <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {selectedItems.length} portfolio{selectedItems.length > 1 ? 's' : ''} selected
            </span>
            <button
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No portfolios found</p>
                      <Link 
                        to="/admin/portfolio/add"
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <User size={18} className="text-[#27CBCB]" />
                        </div>
                        <div>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {item.taglines?.slice(0, 3).map((tag, idx) => (
