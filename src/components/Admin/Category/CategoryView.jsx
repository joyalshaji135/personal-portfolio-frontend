...
const CategoryView = () => {
  ...
  const { currentCategory, isLoading, error, getById, deleteCategory, clearError, clearCurrent } = useCategory();
  ...
  const fetchData = async () => {
    try {
      ...
      setLocalError('Failed to load category');
    }
  };
  ...
  if (error || localError || !currentCategory) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <AlertCircle size={48} className="text-red-400 mx-auto mb-4" />
          <p className="text-red-400">{error || localError || 'Category not found'}</p>
          <button
            onClick={() => {
              clearError();
              navigate('/admin/category');
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <div className="flex gap-2">
          <Link
            to={`/admin/category/edit/${currentCategory._id}`}
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex items-start gap-6">
            ...
            >
              <FolderTree size={40} className="text-[#27CBCB]" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800/30' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Category Name</p>
