...
const CategoryAll = () => {
  const { categories, isLoading, error, getAll, deleteCategory, clearError } = useCategory();
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected categories?`)) {
      setDeleteLoading(true);
      try {
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <FolderTree size={18} className="text-[#27CBCB]" />
        ...
        {selectedItems.length > 0 && (
          ...
          >
            <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {selectedItems.length} category{selectedItems.length > 1 ? 'ies' : ''} selected
            </span>
            <button
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <FolderTree size={48} className="text-gray-600" />
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          to={`/admin/category/view/${item._id}`}
