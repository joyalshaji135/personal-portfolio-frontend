...
const RoleAll = () => {
  const { roles, isLoading, error, getAll, deleteRole, clearError } = useRole();
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected roles?`)) {
      setDeleteLoading(true);
      try {
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Total Roles</span>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Archived</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {roles.filter(r => r.status === 'archived').length}
            </span>
        ...
        {selectedItems.length > 0 && (
          ...
          >
            <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {selectedItems.length} role{selectedItems.length > 1 ? 's' : ''} selected
            </span>
            <button
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No roles found</p>
                      <Link 
                        to="/admin/role/add"
