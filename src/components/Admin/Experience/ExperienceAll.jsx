...
const ExperienceAll = () => {
  const { experiences, isLoading, error, getAll, deleteExperience, clearError } = useExperience();
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected experiences?`)) {
      setDeleteLoading(true);
      try {
        ...
        alert('Failed to delete selected experiences. Please try again.');
      } finally {
        setDeleteLoading(false);
      }
    }
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Companies</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No experiences found</p>
                      <Link 
                        to="/admin/experience/add"
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          to={`/admin/experience/view/${item._id}`}
                        ...
                        <Link
                          to={`/admin/experience/edit/${item._id}`}
                          className={`p-2 rounded-lg transition-colors ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}
                          title="Edit"
                        >
