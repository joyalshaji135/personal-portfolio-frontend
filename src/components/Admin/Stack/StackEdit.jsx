...
const StackEdit = () => {
  ...
  const { currentStack, isLoading, isSubmitting, error, getById, update, clearError, clearSuccess, success } = useStack();
  ...
  useEffect(() => {
    if (currentStack && currentStack._id === id) {
      setFormData({
        title: currentStack.title || '',
        subtitle: currentStack.subtitle || '',
        quote: {
          text: currentStack.quote?.text || '',
          author: currentStack.quote?.author || ''
        },
        categories: currentStack.categories || [],
        others: currentStack.others || []
      });
    }
  }, [currentStack, id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLocalError(err || 'Failed to update stack. Please try again.');
    }
  };
