...
const StackAll = () => {
  const { stacks, isLoading, error, getAll, deleteStack, clearError } = useStack();
  ...
  const filteredData = stacks.filter(item => {
    const matchesSearch = 
      item.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.subtitle?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.quote?.author?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.quote?.text?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected stacks?`)) {
      setDeleteLoading(true);
      try {
        ...
        alert('Failed to delete selected stacks. Please try again.');
      } finally {
        setDeleteLoading(false);
      }
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className={`mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Manage your tech stack entries
          </p>
        </div>
        <Link 
          to="/admin/stack/add"
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Code2 size={18} className="text-[#27CBCB]" />
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Categories</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {stacks.reduce((acc, s) => acc + (s.categories?.length || 0), 0)}
            </span>
          </div>
        </div>
        <div className={`rounded-xl p-4 border
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Quotes</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {stacks.filter(s => s.quote?.text).length}
            </span>
          </div>
        </div>
      </div>
        ...
        {selectedItems.length > 0 && (
          ...
          >
            <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {selectedItems.length} stack{selectedItems.length > 1 ? 's' : ''} selected
            </span>
            <button
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No stacks found</p>
                      <Link 
                        to="/admin/stack/add"
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div>
                        ...
                        {item.quote?.text && (
                          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} truncate max-w-[250px]`}>
                            "{item.quote.text.substring(0, 40)}..."
                          </p>
                        )}
                      </div>
                    </td>
