...
const AboutAll = () => {
  const { abouts, isLoading, error, getAll, deleteAbout, clearError } = useAbout();
  ...
  // Filter and search
  const filteredData = abouts.filter(item => {
    const matchesSearch = 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className={`mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Manage your about section entries
          </p>
        </div>
        <Link 
          to="/admin/about/add"
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Draft</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {abouts.filter(item => 
                !item.status || 
                ...
                <tr>
                  <td colSpan="6" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No about entries found</p>
                      <Link 
                        to="/admin/about/add"
                        className="text-[#27CBCB] hover:text-[#27CBCB]/80 transition-colors text-sm"
                      >
                        Create your first about entry
                      </Link>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden
                          bg-gradient-to-r from-[#27CBCB]/20 to-blue-500/20"
                        >
                          {item.avatarUrl && !imageErrors[item._id] ? (
                            <img 
                              src={item.avatarUrl} 
                              alt={item.name} 
                        ...
                        <div>
                          ...
                          <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'} sm:hidden`}>
                            {item.handle || 'N/A'}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <div className="space-y-1">
                        <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                          <span className="font-mono">{item.handle || 'N/A'}</span>
                        </p>
