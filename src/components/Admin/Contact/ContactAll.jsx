...
const ContactAll = () => {
  const { contacts, isLoading, error, getAll, deleteContact, clearError } = useContact();
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Delete ${selectedItems.length} selected contacts?`)) {
      setDeleteLoading(true);
      try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className={`mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Manage contact section and social links
          </p>
        </div>
        <Link 
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Social Links</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {contacts.reduce((acc, c) => acc + (c.socials?.length || 0), 0)}
            </span>
          </div>
        </div>
        <div className={`rounded-xl p-4 border
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Active</span>
            </div>
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        {selectedItems.length > 0 && (
          ...
          >
            <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {selectedItems.length} contact{selectedItems.length > 1 ? 's' : ''} selected
            </span>
            <button
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>No contacts found</p>
                      <Link 
                        to="/admin/contact/add"
                  ...
                  <tr key={item._id} className={`transition-colors ${isDark ? 'hover:bg-gray-800/30' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} truncate max-w-[200px]`}>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {item.socials?.slice(0, 3).map((social, idx) => (
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          to={`/admin/contact/view/${item._id}`}
                        ...
                        <Link
                          to={`/admin/contact/edit/${item._id}`}
                          className={`p-2 rounded-lg transition-colors ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}
                          title="Edit"
                        >
